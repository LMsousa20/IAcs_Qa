---
title: NF-e de Saída
url: https://acsapps.atlassian.net/wiki/spaces/acsdocumentacao/pages/73105418
---
...
Nota Fiscal Eletrônica de Saída (NF-e de Saída)
===============================================

A **Nota Fiscal Eletrônica de Saída** **(NF-e de Saída)** é um documento fiscal emitido pelo vendedor para documentar a saída de mercadorias para o cliente destinatário, essencial para a transparência e legalidade das transações comerciais. No caso de um posto de combustível, ela registra a venda de combustíveis e mercadorias, e é fundamental para o controle tributário, permitindo o cálculo e a arrecadação de impostos como o ICMS. Além de servir como prova da transação, a nota fiscal de saída fornece informações valiosas para a contabilidade da empresa, facilitando a gestão financeira e garantindo a conformidade com a legislação.

Alguns exemplos de utilização são:

*   **Venda de Produtos**: Quando uma loja vende mercadorias para um cliente, ela emite uma Nota Fiscal de Saída para registrar a operação e formalizar a saída dos produtos do seu estoque.
    
*   **Transferência de Estoque**: Se uma empresa transfere produtos entre suas filiais, ela emite uma Nota Fiscal de Saída para documentar a saída dos bens de um local e a entrega em outro.
    

Resumindo, emite-se quando a mercadoria **sai** do estabelecimento.

:note:atlassian-note#B3D4FF

**Observação:** Para realizar a emissão da Nota Fiscal de Saída (NF-e de Saída), é necessário que um certificado digital válido seja cadastrado no sistema. Para configurá-lo, acesse o menu **Cadastro ->** **Fiscal** \-> **Certificados Digitais**. Além disso, é necessário que o CNPJ esteja ativo e que a empresa possua registro regular na SEFAZ (Secretaria da Fazenda).

Para acessar esse módulo no sistema, basta seguir os seguintes passos:

Menu Movimento > NF-e de Saída
...
Ao acessar o módulo, a tela principal será exibida, permitindo a visualização e a emissão das **notas fiscais de saída (NF-e)** já emitidas. Esta tela é organizada para facilitar a navegação, a consulta e o controle das NF-es.

**Emitir Nova Nota Fiscal de Saída**

Permite registrar a emissão de uma nota fiscal de saída no sistema. Essa funcionalidade deve ser utilizada em situações em que seja necessário documentar a saída de produtos do posto, como nos casos de devolução de compra, transferência de mercadorias entre filiais ou faturamento de convênios, em que seja necessária a emissão de uma nota fiscal de saída para fins de registro fiscal da operação.

**Filtro de Pesquisa**

Facilita a busca da nota fiscal de saída (NF-e) já cadastradas, por meio de filtros como empresa, período, tipo de nota, status da NF-e, cliente, valores entre outros agilizando a localização das informações necessárias.

**Visualização**

Permite consultar os detalhes de uma nota fiscal de saída (NF-e) já cadastrada, possibilitando a visualização das informações fiscais, valores, impostos e dados vinculados ao documento.
...
### Box 1 - Nova Nota Fiscal de Saída

O box **Nova Nota Fiscal de Saída** é onde são inseridos os dados gerais da nota. Esses dados são fundamentais para a emissão da nota fiscal, garantindo conformidade fiscal e organizacional.

Box Nova Nota Fiscal de Saída
...
*   `Tipo da Emissão`: Define o tipo de emissão da nota, podendo ser:
    ...
    *   `Normal`: Utilizada para diversas operações, como vendas, baixas de estoque, saída de ativos imobilizados, comodatos e outras saídas não especificadas (CFOP 5949). Este documento serve como nota de justificativa de saída, formalizando a movimentação de produtos ou ativos e garantindo a precisão nos registros contábeis, além de assegurar a conformidade fiscal.
        
    *   `Devolução de Compra`**:** Nota fiscal de saída utilizada para registrar a devolução de mercadorias ao fornecedor, ajustando estoque e obrigações fiscais.
...
*   `Data`: Indica a data em que a mercadoria saiu do estabelecimento do emitente.
...
*   `Hora`: Hora em que a mercadoria saiu do estabelecimento.
    
*   `Fatura`: Ao marcar esta opção, será liberado o campo para referenciar uma fatura de convênio. Ao selecionar a fatura, as informações do cliente e dos itens são automaticamente importadas para a nota, facilitando o preenchimento e garantindo a consistência dos dados.
    
*   `Ref. Cupom`: Ao marcar esta opção, o usuário deverá selecionar entre as opções de `Seleção de Vendas` ou `Indicação de Cupons` para referenciar os cupons na nota.
...
*   `Modalidade da Nota`: Indica o tipo de operação relacionada à emissão da NF-e. Bloqueado na modalidade **Própria** por padrão.
...
Permite o preenchimento de informações complementares para a saída de mercadorias. É uma interface destinada à gestão detalhada da movimentação fiscal e logística de mercadorias recebidas.
...
*   `Integrado`: No processo integrado, o cliente realiza o pagamento com cartão no momento da criação da nota e essa informação é automaticamente registrada na Nota Fiscal de Saída quando emitida.
...
No box **Itens da Nota** são inseridos e listados os produtos que farão parte da Nota Fiscal que está sendo emitida. Esses itens terão seus saldos atualizados no sistema, permitindo o controle do estoque e a contabilização precisa das saídas. Com isso, o sistema facilita uma gestão eficiente do estoque e garante a conformidade com as obrigações fiscais, assegurando que todas as saídas sejam registradas de forma clara e organizada.

Box Itens da Nota

Os campos apresentados são:

*   `Num.`: Número sequencial de itens listados.
...
*   `Saldo`: Subtração da quantidade de itens já registrada no estoque com a quantidade de itens indicada na nota de saída.
...
*   `Valor CBS (R$)`: Apresenta o valor total de CBS da nota fiscal de saída. O sistema realiza a soma automática dos valores de CBS informados em cada item da nota, exibindo neste campo o total consolidado.
...
*   `Valor CBS Mono (R$)`: Apresenta o **valor total da CBS sujeito ao regime monofásico**, conforme destacado nos itens da Nota Fiscal de Saída. No regime monofásico, a tributação ocorre de forma concentrada em etapa anterior da cadeia (refinaria ou importador), podendo o valor do imposto já estar recolhido na origem.
...
*   `Observação`: Campo de texto livre para inserir informações adicionais ou comentários relevantes sobre o documento ou nota fiscal.
...
*   Botão : Grava o texto informado no campo `Observação`, definindo-o como padrão nas informações complementares da Nota Fiscal de Saída. Após salvo, a informação será automaticamente preenchida nas próximas emissões de notas fiscais. **Para alterar a observação padrão, atualize o texto e clique novamente no botão.**
...
**Observação**: Para que o envio funcione corretamente, acesse o **Menu Opções -> Sistema** **\->** **Guia SMTP -> Guia CONFIGURAÇÕES SMTP** e realize a configuração dos campos necessários.

* * *

Emissão de Nota Fiscal de Saída - Tipo Normal
---------------------------------------------

Alguns benefícios desse tipo de lançamento são:

*   **Conformidade Fiscal**: Garante o cumprimento das obrigações tributárias, evitando penalidades e autuações fiscais, ao registrar oficialmente todas as vendas.
    
*   **Segurança nas Relações Comerciais**: Ao emitir a nota fiscal para cada venda, o posto demonstra profissionalismo e segurança aos clientes, aumentando a confiabilidade nas transações.
    

:note:atlassian-note#B3D4FF

**Observação**: Para emitir a nota fiscal de Saída (NF-e) através deste módulo, é indispensável que o computador utilizado possua um **certificado digital válido** instalado, além de um CNPJ ativo e registro regular na SEFAZ (Secretaria da Fazenda). O certificado digital serve para autenticar a identidade da empresa emissora e garantir a segurança e validade jurídica das transações eletrônicas junto à SEFAZ.

Acesse o módulo de Notas Fiscais de Saída e clique no botão para emitir uma nova nota:

No box Nova Nota Fiscal de Saída, preencha os campos de `Empresa`, `Tipo da Emissão`, `Contingência`, `Emissão`, `Data` e `Hora`.
...
Verifique e corrija, se necessário, os campos `Usar Preço`, `Pagamento`, `Forma de Pagamento` e `Tipo de Frete`.

Com essas informações preenchidas no box **Nova Nota Fiscal de Saída**, prossiga para o próximo box.
...
A tela será redirecionada para a listagem das notas de saída cadastradas no sistema, onde:
...
### Notas de Saída - Faturas de Convênio

Uma das maneiras de incluir os itens na nota de forma mais rápida é por meio da referência de faturas de convênio.

Para referenciar uma fatura de convênio, no box **Nova Nota Fiscal de Saída**, digite o código da fatura no campo Código de Fatura e clique no ícone para que o sistema localize e carregue automaticamente o registro. Caso não saiba o código, clique no ícone para abrir a tela de seleção de faturas. Será apresentada a seguinte tela:

Tela de seleção de fatura de convênio

A tela de seleção de faturas permite visualizar as faturas e escolher aquela que será referenciada na emissão da nota.
...
*   `Data Inicial` e `Data Final`: Filtra as faturas emitidas dentro de um intervalo de datas específico.
    
*   `Situação`: Filtra as faturas de acordo com seus status, podendo ser:
    
    *   `Todas`: Mostra todas as faturas independentemente do status.
        
    *   `Pendentes`: Mostra as faturas que estão com valor pendente de quitação.
        
    *   `Quitadas`: Mostra as faturas que estão com o valor total quitado.
        
*   `Valor Inicial (R$)` e `Valor Final (R$)`: Filtra as faturas de acordo com o intervalo de valores.
...
*   `Código da Fatura`: Filtra a fatura pelo código informado.
...
Após configurar os filtros desejados, clique no botão para atualizar a listagem de faturas de acordo com os critérios aplicados. Marque a fatura que deve ser referenciada na nota fiscal.

:note:atlassian-note#B3D4FF

**Observação:** Somente **uma fatura** de convênio pode ser vinculada a cada nota fiscal.

Na listagem das faturas será possível ver informações como `Emissão`, `Cliente`, `Núm. Fatura`, `Vencimento`, `Situação` e `Valor`.

Guia PRODUTOS DA FATURA1800

*   `Cod. Barras`: Código de barras utilizado para identificar o produto.
    
*   `Produto`: Descrição do produto incluído na venda que originou a fatura.
    
*   `Quantidade`: Quantidade do produto registrada na venda.
    
*   `Valor Contrato (R$)`: Valor do produto definido em contrato para o cliente.
    

Após selecionar a fatura desejada, clique no botão para incluir os itens da fatura na nota fiscal em emissão.

No box de **Itens da Nota**, o(s) produto(s) da fatura será(ão) automaticamente adicionado(s) para o box com suas respectivas informações de valores, quantidade e estoque.
...
Após a verificação e conferência das informações, clique no botão para finalizar a emissão da nota fiscal de saída.

* * *

### Notas de Saída - Referência de Cupons

Outra maneira de incluir os itens na nota de forma ágil é por meio da referenciação de cupons, que pode ser realizada de duas maneiras:

1.  **Seleção de Vendas**
    
2.  **Indicação de Cupons**.
    

Para isso, no box **Nova Nota Fiscal de Saída**, clique no ícone do campo `Ref. Cupom`. Será apresentada a seguinte tela para seleção da forma de referência:

Tela para escolher forma de referenciar os cupons

:note:atlassian-note#B3D4FF

**Observação**: É necessária a indicação do **Cliente** antes de clicar no ícone .

#### **Opção 1 - Seleção de Vendas**

Ao selecionar a opção **Seleção de Vendas**, será apresentada a seguinte tela para indicação das vendas realizadas no PDV que serão referenciadas na nota:
...
*   `Cliente`: O sistema preenche este campo automaticamente com o cliente selecionado no box **Nova Nota Fiscal de Saída** e com o **Cliente Padrão**.
...
**Observação:** Esta opção é habilitada somente quando a caixa de seleção `Vendas pagas apenas com cartão` estiver marcada.
...
Na listagem das vendas será possível ver informações como `Emissão`, `Valor (R$)`, `Modelo`, `Série`, `Cupom`, `Cliente`, `CPF/CNPJ`, Placa e `Chave Eletrônica`.
...
#### **Opção 2 - Indicação dos Cupons**

Ao selecionar a opção **Indicação dos Cupons**, será apresentada a seguinte tela para que o usuário insira manualmente os números dos cupons específicos a serem referenciados, oferecendo maior flexibilidade ao buscar cupons individuais.

Tela de Indicação de cupons

Guia OPÇÕES1800

*   `Data Inicial` e `Data Final`: Indica a data inicial e final de emissão do cupom.
    
*   `Série`: Número identificador dos cupons emitidos.
    
*   `Chave NF-e`: Código único que identifica cada cupom fiscal emitido.
    

Guia LISTA1800

*   `Cupom`: Campo destinado a digitação manual do número do documento do cupom a ser adicionado.
    
*   Botão : Adiciona os cupons digitados na listagem.
    
*   Botão : Remove da listagem o registro com a mesma numeração indicada no campo **Cupom**. Também é possível realizar a exclusão de qualquer cupom clicando no ícone respectivo ao cupom desejado.
    

Logo abaixo, é exibida a listagem dos cupons inseridos.

Após indicar os cupons desejados, clique no botão para adicionar os itens dos mesmos na nota fiscal.

No box de **Itens da Nota**, o(s) produto(s) do(s) cupom(ns) será(ão) automaticamente adicionado(s) para o box com suas respectivas informações de valores, quantidade e estoque.
...
Emissão de Nota Fiscal de Saída - Tipo Devolução de Compra
----------------------------------------------------------

Alguns benefícios desse tipo de lançamento são:

*   **Minimização de Custos**: Evita prejuízos com mercadorias defeituosas, fora das especificações ou que não atendem ao cliente, reduzindo custos de armazenamento e manuseio. Além disso, evita que a empresa tenha que arcar com produtos fora do padrão.
    
*   **Aprimoramento de Controle Logístico**: A emissão de notas fiscais de devolução facilita a organização logística, ajudando a registrar e rastrear a movimentação de mercadorias retornadas. Isso é fundamental para uma gestão eficiente de transporte e armazenamento, melhorando o fluxo de operações.
    

Acesse o módulo de Notas Fiscais de Saída e clique no botão para cadastrar uma nova nota.

No box **Nova Nota Fiscal de Saída**, preencha os campos de `Empresa`, `Tipo da Emissão`, `Contingência`, `Emissão`, `Data` e `Hora`.

Ao selecionar a opção **Devolução de Compra** no campo `Tipo da Emissão`, o campo `Cliente` apenas será preenchido quando a compra a ser devolvida for indicada.

Box Criar Nova Nota Fiscal - Tipo Devolução de Compra

Verifique e corrija, se necessário, os campos **Usar Preço, Pagamento, Forma de Pagamento** e **Tipo de Frete**.

Com essas informações preenchidas no box de **Nova Nota Fiscal de Saída**, prossiga para o próximo box.

Para indicar a compra a ser devolvida, clique no botão localizado no box **Dados Adicionais**. Será apresentada a seguinte tela:

Tela de Seleção de Compra

Na parte superior da tela, é possível aplicar filtros para a listagem dos registros, facilitando a localização e seleção das compras relevantes, onde:

*   `Fornecedor`: Selecione um fornecedor para que apenas as notas emitidas pelo mesmo sejam listadas.
    
*   `Produtos`: Filtra somente as compras que possuam os produtos selecionados nos itens da nota fiscal.
    
*   `Valor Mínimo` e Valor Máximo: Defina o intervalo de valores.
    
*   `Data Inicial` e `Data Final`: Filtra as compras de acordo com o intervalo de valores.
    
*   `Documento`: Filtra a compra específica pelo seu número de documento.
    
*   `Chave Eletrônica`: Filtra a compra específica pela sua chave.
    
*   `Ordem`: Organize os resultados conforme necessário.
    
    *   `Fornecedor/Data`: Organize os resultados por fornecedor e data de compra.
        
    *   `Fornecedor/Valor`: Organize os resultados por fornecedor e valor das compras.
        
    *   `Valor`: Classifique os resultados pelo valor total das compras.
        
*   `Compra contendo apenas os Produtos relacionados`: Se marcado, serão listadas somente compras que possuam os produtos selecionados no filtro Produtos.
    

Após aplicar os filtros, clique no botão para atualizar a listagem de registros. A tela exibirá todas as compras correspondentes aos critérios selecionados. Marque a compra que deve ser referenciada na nota fiscal.

Na listagem das compras será possível ver informações como `Emissão`, `Documento`, `Fornecedor`, `Chave Eletrônica` e `Total Nota (R$)`.

Guia PRODUTOS LANÇADOS1800

*   `Documento`: Número de identificação da compra.
    
*   `Núm.`: Número do item do produto na nota.
    
*   `Cod. Barras`: Código de barras do produto.
    
*   `Produtos`: Lista dos produtos adquiridos.
    
*   `Valor Unitário (R$)`: Preço de cada produto individualmente.
    
*   `Qtd.`: Quantidade de produtos comprados.
    
*   `Valor (R$)`: Total da compra (valor unitário multiplicado pela quantidade).
    

Após marcar a compra desejada, clique no botão para adicionar os itens da mesma na nota fiscal.

No box de **Itens da Nota**, o(s) produto(s) da compra será(ão) automaticamente adicionado(s) para o box com suas respectivas informações de valores, quantidade e estoque.
...
Após a emissão da nota fiscal, o sistema permite a consulta de todas as informações relacionadas à mesma. Para visualizar os detalhes, acesse o módulo de NF-e de Saída, conforme demonstrado no início do manual. A seguinte tela será exibida:

Tela de listagem de notas de saída
...
No box **Alterar Nota Fiscal de Saída**, constam os seguintes botões: , e .
...
**Observação**: O prazo para cancelamento de uma Nota Fiscal de Saída pode variar de acordo com a legislação estadual e as regras específicas de cada estado no Brasil, mas, em geral, o prazo costuma ser de **24 horas** a partir da emissão da nota fiscal.
...
Envio de Notas Fiscais de Saída por E-mail
------------------------------------------
...
Acesse o módulo de Notas Fiscais de Saída e clique no ícone para acessar a nota a ser enviada.
